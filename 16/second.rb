...
Instrument.time do
  ...
  def compute(packet)
    ...
    values = packet[:sub_packets].map do |sub_packet|
      compute(sub_packet)
    end

    return values.sum if packet[:type].zero?
    return values.inject(:*) if packet[:type] == 1
    return values.min if packet[:type] == 2
    return values.max if packet[:type] == 3
    return values[0] > values[1] ? 1 : 0 if packet[:type] == 5
    return values[0] < values[1] ? 1 : 0 if packet[:type] == 6
    return values[0] == values[1] ? 1 : 0 if packet[:type] == 7
  end

  puts compute(packets[0])
end
